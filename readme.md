...
- 0 副作用丢失，0 孤立跳转
- 样本库：`samples/` 现收录 47 个二进制（obpo 真实商业 so + ground truth、
  cdong OLLVM 官方 x86/x64 样本、PDD ARM32、MIPS32/64 平台库、以及从
  example/ APK 复制的 11 个 CFF 阳性 arm64 so），来源见
  `samples/SOURCES.md`，机器可读清单见 `samples/manifest.json`。
...
### 6.5 多维可读性评估（不再硬编码模式选择）

不做「块数/特征 → 固定路径」的规则选择。`tools/eval_modes.py` 对同一函数
在 fresh BinaryView 上实际试跑 auto / general，记录：

- MLIL 块数、边数、圈复杂度、显式分支数；
- HLIL 行数、if / loop / goto / jump / switch / case / call 数；
- 表达式最大嵌套深度；
