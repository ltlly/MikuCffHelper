...
def select_workflow_mode(
    mlil: MediumLevelILFunction, default: str = "auto"
) -> str:
    """变换前选择 auto 或 general 的便宜特征。

    规则（来自 39 函数 baseline 的特征搜索，选择后 38/39 变换且总块数 /
    HLIL 优于纯 auto）：
    1. dispatcher 决策树完全没有 CMP_E（例如 sub_45985c 的 CMP_NE/SGT
       only 变种，auto 失败，general 能处理）→ general；
    2. 函数 ≥70 块且直接状态候选 ≤3（libkste 大函数，general 压缩明显）→
       general；
    3. 其它 → auto。
    """
    de = detect_flattening_candidate(mlil)
    if de is None:
        return default
    const_values, _var_by_id = _collect_const_value_sets(mlil)
    cmp_ids = _collect_comparison_var_ids(mlil, de)
    direct_state_count = sum(
        1 for vid in cmp_ids if len(const_values.get(vid, set())) >= 2
    )
    ...
    while queue:
        ...
        for idx in range(bb.start, bb.end):
            ...
            if not (hasattr(cond, "left") and hasattr(cond, "right")):
                continue
            # 只统计 dispatcher 状态变量的比较；handler 里的 x7_2==2 这类
            # 分支条件不算 dispatcher 特征
            left_var = _var_of(cond.left)
            right_var = _var_of(cond.right)
            if left_var is None and right_var is None:
                continue
            if left_var is not None and left_var.identifier not in cmp_ids:
                continue
            if right_var is not None and right_var.identifier not in cmp_ids:
                continue
            op = cond.operation
    ...
    if direct_state_count == 0:
        # alias-only / 比较变量无常量赋值的变种，auto 检测层会直接失败
        return "general"
    if cmp_e == 0 and cmp_non_e > 0:
        return "general"
    if len(list(mlil.basic_blocks)) >= 70 and direct_state_count <= 3:
        return "general"
    return "auto"
